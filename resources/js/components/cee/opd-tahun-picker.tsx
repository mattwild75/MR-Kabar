...
export function OpdTahunPicker({ routeName, opdOptions, opdId, tahun, opdStatus }: OpdTahunPickerProps) {
    const navigate = (nextOpdId: number | null, nextTahun: number) => {
        router.get(routeName, { opd_id: nextOpdId ?? undefined, tahun: nextTahun }, { preserveState: true, preserveScroll: true, replace: true });
    };
        ...
        <div className="space-y-2">
            <div className="flex flex-wrap items-end gap-3">
                ...
                <div className="w-32 space-y-1">
                    ...
                    <Input type="number" value={tahun} onChange={(e) => navigate(opdId, Number(e.target.value) || tahun)} />
                </div>
            ...
            {opdStatus && Object.keys(opdStatus).length > 0 && (
                <details className="text-xs">
                    ...
                    <div className="bg-muted/30 mt-2 flex flex-wrap gap-2 rounded-md border p-2">
                        {opdOptions.map((opd) => (
                            <button
                                key={opd.id}
                                type="button"
                                onClick={() => navigate(opd.id, tahun)}
                                className={`bg-background hover:bg-muted flex items-center gap-1.5 rounded border px-2 py-1 text-left ${opd.id === opdId ? 'ring-primary ring-1' : ''}`}
                            >
