...
export default function AppLogoIcon(props: SVGAttributes<SVGElement>) {
    ...
    if (!setting?.logo) {
        return (
            <svg xmlns="http://www.w3.org/2000/svg" width="200" height="200" viewBox="0 0 24 24" className={sizeClass}>
                <path
                    fill="currentColor"
                    fillRule="evenodd"
    ...
    const img = <img src={`/storage/${setting.logo}`} alt="App Logo" className={`${sizeClass} object-contain`} />;
    ...
    if (setting.logo_bg) {
        return (
            <span className="inline-flex items-center justify-center rounded p-1" style={{ backgroundColor: setting.logo_bg }}>
                {img}
            </span>
        );
    }
