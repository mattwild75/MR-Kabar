        ...
        <footer className="border-border/70 bg-background/95 text-muted-foreground flex flex-col items-center justify-between gap-3 border-t px-4 py-3 text-sm md:flex-row md:px-6">
            <div className="flex flex-col items-center gap-1 md:items-start">
                <div className="flex flex-wrap items-center justify-center gap-x-2 gap-y-1 md:justify-start">
                    ...
                    {footerCredit && <span className="text-foreground font-medium">{footerCredit}</span>}
                </div>
                <div className="flex items-baseline gap-2">
                    <Link href="/dashboard" className="text-foreground font-semibold hover:underline">
                        MR Kabar
                    </Link>
                    <span className="text-muted-foreground text-xs italic">&ldquo;Risiko TerKabar, Daerah Terjaga&rdquo;</span>
                </div>
            </div>
            ...
            <div className="flex items-center gap-4">
                <DropdownMenu>
                    <DropdownMenuTrigger asChild>
                        <button type="button" className="hover:text-foreground flex items-center gap-1.5 font-medium">
                    ...
                    <DropdownMenuContent align="end">
                        <DropdownMenuItem asChild>
                            <a href={`https://wa.me/${CONTACT_WHATSAPP}`} target="_blank" rel="noopener noreferrer" className="cursor-pointer">
                                <MessageCircle className="mr-2 h-4 w-4" />
                                WhatsApp
                            </a>
                        </DropdownMenuItem>
                        <DropdownMenuItem asChild>
                            <a href={gmailComposeUrl(recipients)} target="_blank" rel="noopener noreferrer" className="cursor-pointer">
