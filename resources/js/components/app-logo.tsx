...
export default function AppLogo() {
    ...
    const img = <img src={`/storage/${imageSource}`} alt="Logo" className="h-8 w-8 rounded-md object-contain" />;
    ...
    return (
        ...
        <div className="flex items-center gap-2 group-data-[collapsible=icon]:gap-0">
            {imageSource ? (
                setting?.logo_bg ? (
                    <span className="inline-flex shrink-0 items-center justify-center rounded-md p-1" style={{ backgroundColor: setting.logo_bg }}>
                        {img}
                    </span>
                ) : (
                ...
                <div className="bg-sidebar-primary text-sidebar-primary-foreground flex aspect-square size-8 shrink-0 items-center justify-center rounded-md">
                    <AppLogoIcon className="text-primary-foreground dark:text-foreground size-[1.375rem] fill-current" />
                </div>
            )}
