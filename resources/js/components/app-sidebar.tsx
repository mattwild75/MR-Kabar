...
import { iconMapper, onIconMapReady, preloadIconMap } from '@/lib/iconMapper';
...
function SidebarFlyout({
    ...
    return createPortal(
        <div
            ...
            className="bg-popover text-popover-foreground w-64 rounded-md border shadow-md"
        >
            {children}
        </div>,
        document.body,
                ...
                if (hasChildren) {
                    ...
                    const triggerButton = (
                        <SidebarMenuButton
                            ref={
                                usesFlyout
                                    ? (el) => {
                                          if (el) anchorRefs.current.set(menu.id, el);
                                          else anchorRefs.current.delete(menu.id);
                                      }
                                    : undefined
                            }
                            type="button"
                            ...
                            className={cn(
                                ...
                                'group-data-[collapsible=icon]:!my-0.5 group-data-[collapsible=icon]:!justify-center group-data-[collapsible=icon]:!border-l-0 group-data-[collapsible=icon]:!p-2',
                            )}
                        >
                            <div className="flex min-w-0 items-start group-data-[collapsible=icon]:min-w-0">
                                <Icon
                                    className={cn(
                                        'mt-0.5 mr-3 size-4 shrink-0 opacity-90 group-hover:opacity-100 group-data-[collapsible=icon]:mt-0 group-data-[collapsible=icon]:mr-0',
                                        color.icon,
                                    )}
                                />
                                <span className="overflow-visible text-left break-words text-clip whitespace-normal group-data-[collapsible=icon]:hidden">
                                    {menu.title}
                                </span>
                            </div>
                            <ChevronDown
                                className={cn(
                                    'mt-0.5 size-4 shrink-0 opacity-50 transition-transform duration-[250ms] ease-in-out group-hover:opacity-70 group-data-[collapsible=icon]:hidden',
                                    isOpen && 'rotate-180',
                                )}
                            />
                        </SidebarMenuButton>
                    ...
                    if (usesFlyout) {
                        return (
                            <SidebarMenuItem key={menu.id}>
                                {triggerButton}
                                <SidebarFlyout
                                    getAnchor={() => anchorRefs.current.get(menu.id) ?? null}
                                    open={isFlyoutOpen}
                                    onClose={() => setOpenFlyoutId(null)}
                                >
                                    <div className={cn('border-b px-3 py-2 text-xs font-semibold tracking-wide uppercase', color.icon)}>
                                        {menu.title}
                                    </div>
                  ...
                  per grup), tanpa perlu ukur tinggi manual pakai JS. */}
                      ...
                      berbagai tema warna sidebar tanpa perlu var CSS baru. */}
                                    <div className="border-border/70 my-1 ml-1.5 rounded-md border bg-black/5 p-1.5 shadow-sm dark:bg-white/10">
                ...
                return (
                    ...
                    membuat jaraknya lebih lebar dari menu lain (mis. Dashboard). */}
                                <div className="flex min-w-0 items-start group-data-[collapsible=icon]:min-w-0">
                                    ...
                                    />
                                    <span className="!truncate-none text-left break-words whitespace-normal group-data-[collapsible=icon]:hidden">
                                        {menu.title}
                                    </span>
                                </div>
                                {level > 0 && (
                                    <ChevronRight className="mt-0.5 ml-auto size-4 shrink-0 opacity-40 group-hover:opacity-70 group-data-[collapsible=icon]:hidden" />
                                )}
                            </Link>
                        </SidebarMenuButton>
                    </SidebarMenuItem>
...
export function AppSidebar() {
    ...
    return (
        <Sidebar
            collapsible="icon"
            variant="inset"
            className="border-sidebar-border bg-sidebar/95 supports-[backdrop-filter]:bg-sidebar/60 border-r backdrop-blur"
        >
          ...
          mengganggu positioning fixed sidebar. */}
            <SidebarHeader className="border-sidebar-border relative overflow-hidden border-b px-4 py-3 group-data-[collapsible=icon]:px-2">
                <div
                    className="from-primary via-primary/70 to-primary/20 pointer-events-none absolute inset-x-0 top-0 h-1.5 bg-gradient-to-r"
                    aria-hidden="true"
                />
                <div className="from-primary/[0.08] pointer-events-none absolute inset-0 bg-gradient-to-b to-transparent" aria-hidden="true" />
                <SidebarMenu>
                    <SidebarMenuItem>
