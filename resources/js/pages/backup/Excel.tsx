                ...
                <Card>
                    <CardHeader>
                        ...
                        <p className="text-muted-foreground text-sm">
                            Satu file Excel berisi {modules.length} tab (satu tab per modul Form Input), mencakup seluruh data lintas-OPD. Format
                            Excel (nama tab, urutan &amp; nama kolom) tidak boleh diubah — hanya isian datanya. Khusus Admin/Super Admin.
                        </p>
                    </CardHeader>
                    <Separator />
                    <CardContent className="space-y-4 pt-4">
                        <ul className="text-muted-foreground grid grid-cols-1 gap-1 text-sm sm:grid-cols-2">
                            {modules.map((m) => (
                                <li key={m.slug}>
                ...
                <Card>
                    <CardHeader>
                        ...
                        <p className="text-muted-foreground text-sm">
                            ...
                            terisi akan memperbarui data yang sudah ada. Baris yang ada di database tapi tidak muncul di file tidak akan dihapus.
                        </p>
                        <p className="text-sm text-amber-600 dark:text-amber-400">
                            Khusus akun Admin (bukan Super Admin): impor tidak langsung diproses — file akan menunggu persetujuan Super Admin terlebih
                            dahulu.
                        </p>
                    </CardHeader>
                    <Separator />
                        ...
                        <CardContent className="space-y-3 pt-4">
                            {pendingRequests.map((r) => (
                                <PendingRequestCard
                                    key={r.id}
                                    request={r}
                                    approveUrl={`/backup/excel/import-requests/${r.id}/approve`}
                                    rejectUrl={`/backup/excel/import-requests/${r.id}/reject`}
                                />
                            ))}
                        </CardContent>
                    </Card>
