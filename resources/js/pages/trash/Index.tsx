                    ...
                    {tabs.map((tab) => {
                        ...
                        return (
                            <button
                                ...
                                className={`flex shrink-0 items-center gap-2 rounded-md border px-3 py-1.5 text-sm whitespace-nowrap transition-colors ${
                                    isActive ? 'border-white bg-white font-medium text-black shadow-sm' : 'hover:bg-muted/50'
                                }`}
                            >
                                {tab.label}
                                {tab.count > 0 && (
                                    <Badge variant={isActive ? 'default' : 'secondary'} className={`ml-1 ${isActive ? 'bg-black text-white' : ''}`}>
                                        {tab.count}
                                    </Badge>
                                )}
                            </button>
                                            ...
                                            <div className="min-w-0 flex-1">
                                                ...
                                                {row.deleted_at && (
                                                    <p className="text-muted-foreground mt-1 text-xs">
                                                        Dihapus: {formatTanggalWaktu(row.deleted_at)}
                                                    </p>
                                                )}
                                            </div>
                                                    ...
                                                    <AlertDialog>
                                                        ...
                                                        <AlertDialogContent>
                                                            <AlertDialogHeader>
                                                                ...
                                                                <AlertDialogDescription>
                                                                    Data "{row.title || '—'}" akan dihapus <b>selamanya</b> dan tidak dapat dipulihkan
                                                                    lagi. Tindakan ini tidak bisa dibatalkan.
                                                                </AlertDialogDescription>
                                                            </AlertDialogHeader>
                                                            <AlertDialogFooter>
                                    ...
                                    if (group.batch && group.rows.length > 1) {
                                        return (
                                            <div key={group.batch} className="bg-muted/20">
                                                ...
                                                <div className="divide-y border-l-4 border-l-amber-500/40">{group.rows.map(rowNode)}</div>
                                            </div>
                                        );
                                    }
