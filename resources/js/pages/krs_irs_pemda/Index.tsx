...
// skalaBadgeClass() di irs/Index.tsx dan ikut berubah kalau Admin mengedit
// Level Risiko di Settings > Keterangan Pendukung.
...
function ProgramItem({
    ...
    registerRef,
    riskLevels,
}: { program: ProgramGroup } & Omit<TreeCommonProps, 'activeAncestorKeys' | 'expanded' | 'onToggle'>) {
        ...
        <Collapsible open={open} onOpenChange={setOpen}>
            ...
            >
                <CollapsibleTrigger className="hover:bg-muted/50 flex w-full items-start gap-2 rounded-md px-3 py-2 text-left">
                    <ChevronRight
                        className={`mt-0.5 h-4 w-4 shrink-0 transition-transform ${LEVEL_COLORS.program.chevron} ${open ? 'rotate-90' : ''}`}
                    />
                    ...
                    {program.risikoRows.length > 1 && (
                        <Badge variant="outline" className="shrink-0">
                            {program.risikoRows.length} risiko
                        </Badge>
                    )}
                    {program.risikoRows.length === 1 &&
                        (() => {
            ...
            <CollapsibleContent className="space-y-4 px-3 pb-3 pl-9">
                <section className="space-y-2">
                    <h4 className="text-muted-foreground text-xs font-semibold tracking-wide uppercase">Program Prioritas</h4>
...
function MisiCard({
    ...
    registerRef,
    riskLevels,
}: { group: MisiGroup } & TreeCommonProps) {
    ...
    return (
        <Collapsible open={open} onOpenChange={(o) => onToggle(key, o)}>
            <div
                ref={(el) => registerRef(key, el)}
                className={`rounded-md border-l-4 ${LEVEL_COLORS.misi.border} ${isCurrent ? 'ring-2 ring-orange-500' : ''}`}
            >
                <CollapsibleTrigger className="hover:bg-muted/30 flex w-full items-center gap-3 p-3 text-left">
...
function VisiCard({
    ...
    registerRef,
    riskLevels,
}: { group: VisiGroup } & TreeCommonProps) {
...
export default function KrsIrsPemdaIndex({ rows, riskLevels }: PageProps) {
    ...
    const countPrograms = (groups: VisiGroup[]) =>
        groups.reduce(
            (n, v) =>
                n + v.misis.reduce((n2, m) => n2 + m.tujuans.reduce((n3, t) => n3 + t.sasarans.reduce((n4, s) => n4 + s.programs.length, 0), 0), 0),
            0,
        );
    ...
    return (
        <AppLayout>
            ...
            <div className="space-y-4 p-4">
                ...
                <RiskSummaryCard groups={riskSummaryGroups} riskLevels={riskLevels} title="Rangkuman Risiko per Sasaran RPJMD" />
                ...
                {!activeQuery && <p className="text-muted-foreground text-sm">Menampilkan {totalPrograms} program.</p>}
