...
import ImportResultPanel, { statusBadge, type ImportRequestStatus, type ImportResult, type RequestUser } from '@/components/ui/import-result-panel';
...
export default function KrsExcel({ modules, pendingRequests, myRequests }: Props) {
    ...
    const [selected, setSelected] = useState<Record<string, boolean>>(() => Object.fromEntries(modules.map((m) => [m.slug, true])));
            ...
            <div className="space-y-4 p-4 md:p-6">
                <Card>
                    <CardHeader>
                        ...
                        <p className="text-muted-foreground text-sm">
                            Satu file Excel bisa mencakup ketiga tingkat hierarki sekaligus (KRS Pemda → KRS PD → KRO PD), supaya sekali input
                            langsung menyambungkan ketiganya. KRS Pemda bersifat referensi bacaan saja (data seluruh Pemda) — hanya KRS PD dan KRO PD
                            yang benar-benar bisa diimpor dan hanya berisi data milik Anda sendiri.
                        </p>
                    </CardHeader>
                    <Separator />
                                ...
                                return (
                                    <div key={m.slug} className="flex items-center gap-2">
                                        ...
                                        <Label htmlFor={`mod-${m.slug}`} className="cursor-pointer text-sm">
                                            {m.label} <span className="text-muted-foreground text-xs">({m.sheet_name})</span>
                                            {!m.writable && (
                                                <span className="text-muted-foreground ml-1.5 text-xs">— referensi saja, tidak bisa diimpor</span>
                                            )}
                                            {lockedByChild && (
                ...
                <Card>
                    <CardHeader>
                        ...
                        <p className="text-muted-foreground text-sm">
                            Upload file hasil unduhan Ekspor/Template yang sudah diisi. Baris dengan ID kosong dianggap data baru; baris dengan ID
                            terisi akan memperbarui data Anda yang sudah ada. Baris di KRS Pemda tidak pernah diimpor (referensi saja) — hanya KRS PD
                            dan KRO PD yang akan tertulis.
                        </p>
                    ...
                    <CardContent className="space-y-3 pt-4">
                        <input
                            ...
                            className="text-muted-foreground file:bg-background hover:file:bg-muted block w-full text-sm file:mr-4 file:rounded-md file:border file:px-3 file:py-1.5 file:text-sm file:font-medium"
                        />
...
function PendingRequestCard({ request, approveUrl, rejectUrl }: { request: ImportRequestItem; approveUrl: string; rejectUrl: string }) {
    ...
    return (
        <div className="space-y-3 rounded-md border p-3">
            <div className="flex flex-wrap items-center justify-between gap-2">
                <div>
                    ...
                    <div className="text-muted-foreground text-xs">
                        Diajukan oleh {request.user?.name ?? '-'} (@{request.user?.username ?? '-'}) &middot; {formatTanggalWaktu(request.created_at)}
                    </div>
                </div>
            ...
            {rejecting && (
                <div className="space-y-2">
                    <Textarea value={reason} onChange={(e) => setReason(e.target.value)} placeholder="Alasan penolakan (opsional)..." rows={2} />
                ...
                <div>
                    ...
                    <div className="text-muted-foreground text-xs">
                        ...
                        {request.reviewed_at && (
                            <>
                                {' '}
                                &middot; ditinjau oleh {request.reviewer?.name ?? '-'} pada {formatTanggalWaktu(request.reviewed_at)}
                            </>
                        )}
                    </div>
                    {request.status === 'rejected' && request.rejection_reason && (
                        <div className="text-destructive mt-1 text-xs">Alasan: {request.rejection_reason}</div>
                    )}
                </div>
