        ...
        <div className="overflow-hidden rounded-md border">
            <table className="w-full text-sm">
                <thead className="bg-muted/50">
                    <tr>
                        <th className="text-muted-foreground px-3 py-2 text-left font-medium">{label}</th>
                ...
                <tbody>
                    {Array.from({ length: rowCount }).map((_, i) => (
                        <tr key={i} className="border-t align-top">
                            ...
                            <td className="text-muted-foreground px-3 py-2">
                                {opdLines[i] ? <HighlightText text={opdLines[i]} query={query} /> : '-'}
                            </td>
                        </tr>
                    ))}
                </tbody>
...
function keyOf(kind: 'sasaranrpjmd' | 'tujuan' | 'sasaranpd' | 'sasaranrenstra' | 'program' | 'kegiatan' | 'subkegiatan', id: string): string {
    return `${kind}:${id}`;
}
...
function kegiatanMatches(kegiatan: KegiatanGroup, q: string): boolean {
    ...
    return (
        rowMatches(kegiatan.row) ||
        kegiatan.risikoRows.some(rowMatches) ||
        kegiatan.subkegiatans.some((sk) => sk.subkegiatan.toLowerCase().includes(q))
    );
}
    ...
    return (
        <Collapsible open={open} onOpenChange={setOpen}>
            <div
                ...
                className={`rounded-md border-l-4 ${LEVEL_COLORS.subkegiatan.border} ${isCurrent ? 'ring-2 ring-orange-500' : ''}`}
            >
                <CollapsibleTrigger className="hover:bg-muted/50 flex w-full items-start gap-2 rounded-md px-3 py-2 text-left">
                    <ChevronRight
                        className={`mt-0.5 h-4 w-4 shrink-0 transition-transform ${LEVEL_COLORS.subkegiatan.chevron} ${open ? 'rotate-90' : ''}`}
                    />
                    ...
                    {kegiatan.risikoRows.length > 1 && (
                        <Badge variant="outline" className="shrink-0">
                            {kegiatan.risikoRows.length} risiko
                        </Badge>
                    )}
                    {kegiatan.risikoRows.length === 1 &&
                        (() => {
                            const skalaRisiko = kegiatan.risikoRows[0].SKALA_RISIKO != null ? Number(kegiatan.risikoRows[0].SKALA_RISIKO) : null;
                            return skalaRisiko !== null ? (
                                <Badge className={`shrink-0 ${skalaRisikoColor(skalaRisiko, riskLevels)}`}>Skala {skalaRisiko}</Badge>
                            ) : null;
                        })()}
                </CollapsibleTrigger>
            </div>
            ...
            <CollapsibleContent className="space-y-4 px-3 pb-3 pl-9">
                <section className="space-y-2">
                    <h4 className="text-muted-foreground text-xs font-semibold tracking-wide uppercase">Kegiatan</h4>
                    <div className="bg-muted/30 grid grid-cols-1 gap-3 rounded-md border p-3 sm:grid-cols-2">
                ...
                {kegiatan.subkegiatans.length > 0 && (
                    <section className="space-y-2">
                        <h4 className="text-muted-foreground text-xs font-semibold tracking-wide uppercase">SubKegiatan</h4>
                        <ul className="bg-muted/30 space-y-1 rounded-md border p-3 text-sm">
                ...
                {kegiatan.risikoRows.map((risikoRow, i) => {
                    ...
                    return (
                        <section key={i} className="space-y-2">
                            ...
                            <div className="bg-muted/30 grid grid-cols-1 gap-3 rounded-md border p-3 sm:grid-cols-3">
...
function ProgramRow({
    ...
    registerRef,
    riskLevels,
}: { group: ProgramGroup } & TreeCommonProps) {
    ...
    return (
        <Collapsible open={open} onOpenChange={(o) => onToggle(key, o)}>
            <div
                ref={(el) => registerRef(key, el)}
                className={`rounded-md border-l-4 ${LEVEL_COLORS.kegiatan.border} ${isCurrent ? 'ring-2 ring-orange-500' : ''}`}
            >
                <CollapsibleTrigger className="hover:bg-muted/50 flex w-full items-center gap-2 rounded-md px-3 py-2 text-left">
...
function SasaranRenstraRow({
    ...
    return (
        <Collapsible open={open} onOpenChange={(o) => onToggle(key, o)}>
            <div
                ref={(el) => registerRef(key, el)}
                className={`rounded-md border-l-4 ${LEVEL_COLORS.program.border} ${isCurrent ? 'ring-2 ring-orange-500' : ''}`}
            >
                <CollapsibleTrigger className="hover:bg-muted/50 flex w-full items-center gap-2 rounded-md px-3 py-2 text-left">
...
function SasaranPdRow({
    ...
    return (
        <Collapsible open={open} onOpenChange={(o) => onToggle(key, o)}>
            <div
                ref={(el) => registerRef(key, el)}
                className={`rounded-md border-l-4 ${LEVEL_COLORS.sasaran.border} ${isCurrent ? 'ring-2 ring-orange-500' : ''}`}
            >
                <CollapsibleTrigger className="hover:bg-muted/50 flex w-full items-center gap-2 rounded-md px-3 py-2 text-left">
...
function TujuanRow({
    ...
    registerRef,
    riskLevels,
}: { group: TujuanGroup } & TreeCommonProps) {
    ...
    return (
        <Collapsible open={open} onOpenChange={(o) => onToggle(key, o)}>
            <div
                ref={(el) => registerRef(key, el)}
                className={`rounded-md border-l-4 ${LEVEL_COLORS.tujuan.border} ${isCurrent ? 'ring-2 ring-orange-500' : ''}`}
            >
                <CollapsibleTrigger className="hover:bg-muted/50 flex w-full items-center gap-2 rounded-md px-3 py-2 text-left">
...
function SasaranRpjmdCard({
    ...
    registerRef,
    riskLevels,
}: { group: SasaranRpjmdGroup } & TreeCommonProps) {
    ...
    return (
        <Card className={`overflow-hidden border-l-4 ${LEVEL_COLORS.visi.border} shadow-sm`}>
            <Collapsible open={open} onOpenChange={(o) => onToggle(key, o)}>
                <div ref={(el) => registerRef(key, el)} className={isCurrent ? 'rounded-md ring-2 ring-orange-500' : ''}>
            ...
            (n, sr) =>
                ...
                sr.tujuans.reduce(
                    (n2, t) =>
                        n2 +
                        t.sasarans.reduce(
                            (n3, s) => n3 + s.sasaranRenstras.reduce((n4, sre) => n4 + sre.programs.reduce((n5, p) => n5 + p.kegiatans.length, 0), 0),
                            0,
                        ),
                    0,
                ),
    ...
    return (
        <AppLayout>
            ...
            <div className="space-y-4 p-4">
                ...
                {!activeQuery && <p className="text-muted-foreground text-sm">Menampilkan {totalKegiatans} kegiatan.</p>}
