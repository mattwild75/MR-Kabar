...
function Baris({ label, children, highlight }: { label: string; children: React.ReactNode; highlight?: boolean }) {
    return (
        <tr>
            ...
            <td className={`border border-black p-1.5 align-top ${highlight ? 'bg-yellow-100 font-semibold uppercase' : ''}`}>{children}</td>
        </tr>
    );
}
        ...
        <table className="mt-1 w-full table-fixed border-collapse border border-black text-[11px]">
            ...
            <thead>
                <tr className="text-left">
                    <th className="border border-black px-2 py-1 font-semibold">Indikator</th>
                    <th className="border border-black px-2 py-1 font-semibold">Baseline</th>
                    <th className="border border-black px-2 py-1 font-semibold">Target</th>
                    <th className="border border-black px-2 py-1 font-semibold">OPD</th>
                </tr>
            </thead>
            <tbody>
                {rows.map((r, i) => (
                    ...
                    <tr key={i} className={i > 0 ? 'border-t border-dotted border-black/50' : ''}>
                        <td className="border-x border-black px-2 py-1 align-top">{clean(r.ik)}</td>
                        <td className="border-x border-black px-2 py-1 align-top">
                            {clean(r.baseline)} {clean(r.satuan) !== '-' ? r.satuan : ''}
                        </td>
                        <td className="border-x border-black px-2 py-1 align-top">
                            {clean(r.target)} {clean(r.satuan) !== '-' ? r.satuan : ''}
                        </td>
                        <td className="border-x border-black px-2 py-1 align-top">{clean(r.opd)}</td>
                    </tr>
                ))}
            </tbody>
        </table>
    ...
    return (
        <AppLayout>
            ...
            <div className="space-y-4 p-4 print:hidden">
                <div>
                    <h1 className="text-xl font-semibold tracking-tight md:text-2xl">2a_Konteks Risiko Strategis Pemda</h1>
            ...
            {!konteks ? (
                <div className="text-muted-foreground p-4 text-sm print:hidden">
                        ...
                        <tbody>
                            <tr>
                                ...
                                <td className="border border-black p-1.5 align-top">
                                    ...
                                    <p className="text-muted-foreground mt-1 text-[10px] italic">
                                        *Ket. yang dicetak Tebal : IK dipilih sebagai IKU dan Sasaran RPJMD yang dipilih sebagai Penetapan Konteks
                    ...
                    <table className="mt-3 w-full border-collapse border border-black text-xs">
                        <tbody>
                            <tr>
                                ...
                                <td className="w-4 border border-black p-1.5 text-center align-top">:</td>
                                <td className="space-y-2 border border-black p-1.5 align-top">
                                    {konteks.misi_list
                                        .filter((m) => m.bold)
                                        .map((m) => (
                                            <div key={m.nomor}>
                                    ...
                                    {boldSasaran.length === 0 && (
                                        <span className="text-muted-foreground italic">Belum ada risiko strategis teregister.</span>
                                    )}
                                </td>
                            </tr>
                        </tbody>
