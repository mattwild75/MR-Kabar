    ...
    return (
        <AppLayout>
            ...
            <div className="space-y-4 p-4 print:hidden">
                <div>
                    <h1 className="text-xl font-semibold tracking-tight md:text-2xl">Form 1a — Rekapitulasi Kuesioner CEE</h1>
                ...
                <div className="flex flex-wrap items-end justify-between gap-3">
                    ...
                    {opd && <UnduhPdfButton href={`/cetak/cee/1a/pdf?opd_id=${opd.id}&tahun=${tahun}`} />}
                </div>
            </div>
            ...
            {opd && (
                <div className="cee-print-sheet mx-auto max-w-4xl bg-white p-8 text-black print:m-0 print:max-w-none print:p-0 print:shadow-none">
                    ...
                    <p className="text-muted-foreground text-center text-xs uppercase">{opd.nama}</p>
                    <p className="mt-2 text-sm">Tahun Penilaian : {tahun}</p>

                    <table className="mt-3 w-full border-collapse border border-black text-xs">
                        <thead>
                            <tr>
                                <th className="w-8 border border-black p-1" rowSpan={2}>
                                    No.
                                </th>
                                <th className="border border-black p-1" rowSpan={2}>
                                    Pertanyaan / Kuesioner
                                </th>
                                <th className="border border-black p-1" colSpan={kolomResponden}>
                                    Jawaban Responden (R)
                                </th>
                                <th className="w-14 border border-black p-1" rowSpan={2}>
                                    Modus
                                </th>
                                <th className="w-24 border border-black p-1" rowSpan={2}>
                                    Simpulan Kuesioner CEE
                                </th>
                            </tr>
                            <tr>
                                {Array.from({ length: kolomResponden }, (_, i) => (
                                    <th key={i} className="w-8 border border-black p-1">
                                        R{i + 1}
                                    </th>
                                ))}
                            </tr>
                            <tr className="bg-muted/40">
                                ...
                                <th className="border border-black p-1 font-normal" colSpan={kolomResponden + 1}>
                                    c
                                </th>
                                    ...
                                    {unsur.pertanyaan.map((p, idx) => {
                                        ...
                                        return (
                                            <tr key={p.id}>
                                                ...
                                                <td className="border border-black p-1 text-center align-top font-semibold">{r?.modus ?? ''}</td>
                                                <td
                    ...
                    <div className="mt-4 text-[10px] leading-tight">
                        ...
                        <p className="mt-1">
                            Simpulan tiap pertanyaan: "Memadai" apabila modus jawaban responden adalah 3 atau 4, dan "Kurang Memadai" apabila modus
                            jawaban responden adalah 1 atau 2. Simpulan sub unsur: "Memadai" apabila seluruh simpulan tiap pertanyaan pada sub unsur
                            tersebut "Memadai", dan "Kurang Memadai" apabila terdapat simpulan pertanyaan pada sub unsur tersebut yang "Kurang
                            Memadai".
                        </p>
                    </div>
                </div>
