            ...
            <CardContent className="space-y-3">
                <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                    <div className="rounded-md border p-3">
                        <p className="text-muted-foreground mb-1 text-xs font-medium">Hasil Survei Persepsi (1a)</p>
                        {badgeFor(r.simpulan_1a)}
                    </div>
                    <div className="rounded-md border p-3">
                        <p className="text-muted-foreground mb-1 text-xs font-medium">Hasil Reviu Dokumen (1b)</p>
                ...
                {r.bertentangan && (
                    <div className="rounded-md border border-amber-500/60 bg-amber-50 p-3 text-sm dark:bg-amber-950/30">
                        <p className="font-medium text-amber-900 dark:text-amber-200">Kedua sumber bertentangan</p>
                        <p className="mt-1 text-amber-800 dark:text-amber-300">
                            Reviu dokumen menyimpulkan <strong>{hasilDokumen}</strong>, sedangkan survei persepsi menyimpulkan{' '}
                            <strong>{r.simpulan_1a}</strong>. Perdep meminta simpulannya ditarik lewat pendalaman &mdash; misalnya wawancara atau
                            konfirmasi &mdash; atau lewat professional judgement. Tuliskan dasarnya pada Penjelasan di bawah, apa pun simpulan yang
                            Anda pilih.
                        </p>
                    </div>
                )}
                ...
                {editing ? (
                    ...
                    <div className="space-y-1">
                        <div className="flex items-center gap-2">
                            <p className="text-muted-foreground text-xs font-medium">Simpulan:</p>
                            {badgeFor(tersimpan?.simpulan ?? null)}
                        </div>
                        {tersimpan?.penjelasan && <p className="text-muted-foreground text-sm">{tersimpan.penjelasan}</p>}
                    </div>
                )}
            </CardContent>
        </Card>
...
export default function Form1c({ opdOptions, opdStatus, opdId, tahun, ringkasan, simpulanTersimpan, defaultPenyusun, defaultKepalaOpd }: PageProps) {
    ...
    return (
        <AppLayout>
            ...
            <div className="space-y-4 p-4">
                ...
                {!opdId || !ringkasan ? (
                    <Card>
                        <CardContent className="text-muted-foreground p-8 text-center text-sm">Pilih OPD terlebih dahulu.</CardContent>
                    </Card>
                ) : (
                    <div className="space-y-4">
                        {readOnly && (
                            <p className="rounded-md border border-amber-300 bg-amber-50 p-3 text-sm text-amber-800">
                                Simpulan OPD & tahun ini sudah tersimpan. Akun survei CEE tidak dapat mengeditnya — hubungi PIC OPD, Admin, atau Super
                                Admin.
                            </p>
                        )}
                        <Card>
                            ...
                            <CardContent className="grid grid-cols-1 gap-6 sm:grid-cols-2">
                                ...
                                <div className="space-y-3">
                                    <p className="text-muted-foreground text-xs font-semibold uppercase">Penyusun (Sekretaris/setingkat)</p>
                                    <div className="space-y-1">
                                        <div className="flex items-center gap-1.5">
                                        ...
                                        <Input
                                            ...
                                            disabled={readOnly}
                                        />
                                        {errors.penyusun_nama && <p className="text-destructive text-sm">{errors.penyusun_nama}</p>}
                                    </div>
                                    <div className="space-y-1">
                                        <div className="flex items-center gap-1.5">
                                        ...
                                        <Input
                                            ...
                                            disabled={readOnly}
                                        />
                                        {errors.penyusun_jabatan && <p className="text-destructive text-sm">{errors.penyusun_jabatan}</p>}
                                    </div>
                                </div>

                                {/* Sisi KANAN = Kepala OPD (penandatangan/UPR) — sinkron 2 arah dgn DataUmum.nama_kepala_dinas/jabatan_kepala_dinas & penandatangan[]. */}
                                <div className="space-y-3">
                                    <p className="text-muted-foreground text-xs font-semibold uppercase">Kepala OPD</p>
                                    <div className="space-y-1">
                                ...
                                <p className="text-muted-foreground text-xs sm:col-span-2">
                                    Isi Penyusun & Kepala OPD sebelum menyimpan simpulan unsur mana pun di bawah — keduanya otomatis tersinkron dua
                                    arah dengan menu <b>Data Umum</b> (bagian Penanda Tangan) milik OPD ini.
                                </p>
                            </CardContent>
                        </Card>
