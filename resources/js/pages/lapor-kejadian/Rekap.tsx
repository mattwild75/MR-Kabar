...
export default function LaporKejadianRekap({ laporan, filters, opdList, statuses, isAdminOrSuperAdmin }: Props) {
    ...
    const tautkanRisikoTerdaftar = (r: RisikoHasil) => {
        ...
        router.put(
            `/lapor-kejadian/rekap/${detail.id}/risiko-terdaftar`,
            {
            ...
            },
            {
                ...
                onSuccess: () => {
                    ...
                    setDetail((prev) =>
                        prev ? { ...prev, risiko_terdaftar_tipe: r.tipe, risiko_terdaftar_id: r.id, risiko_terdaftar_uraian: r.uraian_risiko } : prev,
                    );
                    setCariRisikoQuery('');
                    setCariRisikoHasil([]);
                },
        ...
        router.put(
            ...
            },
            {
                ...
                onSuccess: () => {
                    ...
                    setDetail((prev) =>
                        prev ? { ...prev, risiko_terdaftar_tipe: null, risiko_terdaftar_id: null, risiko_terdaftar_uraian: null } : prev,
                    );
                },
    ...
    const simpanOpd = () => {
        ...
        setSavingOpd(true);
        router.put(
            `/lapor-kejadian/rekap/${detail.id}/opd`,
            {
                opd_id: opdEdit || null,
            },
            {
    ...
    const simpanTindakLanjut = () => {
        if (!detail) return;
        router.put(
            `/lapor-kejadian/rekap/${detail.id}/status`,
            {
                    ...
                    {detail && (
                        <div className="space-y-4">
                            <div className="space-y-1 text-sm">
                                <p>
                                    <span className="text-muted-foreground">Pelapor:</span> {detail.nama_lengkap}
                                </p>
                                {detail.email && (
                                    <p>
                                ...
                                {detail.no_hp && (
                                    <p>
                                        <span className="text-muted-foreground">No. HP:</span> {detail.no_hp}
                                    </p>
                                )}
                                {!isAdminOrSuperAdmin && detail.opd && (
                                    <p>
                                        <span className="text-muted-foreground">OPD:</span> {detail.opd.nama}
                                    </p>
                                )}
                                <p>
                                    <span className="text-muted-foreground">Waktu Kejadian:</span> {detail.waktu_kejadian}
                                </p>
                                {detail.tempat && (
                                    <p>
                                        <span className="text-muted-foreground">Tempat:</span> {detail.tempat}
                                    </p>
                                )}
                            </div>
                            ...
                            {isAdminOrSuperAdmin && (
                                <div className="space-y-1.5">
                                    ...
                                    <p className="text-muted-foreground text-xs">
                                        Field ini opsional saat pelapor mengisi form — lengkapi/koreksi di sini. PIC OPD yang dipilih akan otomatis
                                        dinotifikasi.
                                    </p>
                            ...
                            <div>
                                <Label className="text-muted-foreground text-xs">Kejadian</Label>
                                ...
                                <div className="rounded-md border border-dashed p-2">
                                    ...
                                    <p className="text-sm">
                                        {detail.risiko_terdaftar_uraian ?? (
                                            <span className="text-muted-foreground italic">Data risiko tidak ditemukan (mungkin sudah dihapus).</span>
                                        )}
                                    </p>
                                </div>
                            )}
                            ...
                            {detail.risiko_terdaftar_tipe ? (
                                <div className="space-y-2 border-t pt-3">
                                    ...
                                    <p className="text-muted-foreground text-xs">
                                        Laporan ini sudah tertaut ke risiko yang sudah ada — buka baris risikonya langsung untuk ditinjau/dilengkapi,
                                        tidak perlu input ulang dari form baru.
                                    </p>
                                ...
                                <div className="space-y-2 border-t pt-3">
                                    ...
                                    <p className="text-muted-foreground text-xs">
                                        Buka form terkait di tab baru dengan Uraian Risiko/OPD/Pemicu sudah terisi otomatis dari laporan ini —
                                        lengkapi sisa penilaian risiko lalu simpan seperti biasa.
                                    </p>
                  ...
                  redundan tampil berdampingan dgn kartu info yg sudah ada. */}
                            {detail.risiko_terdaftar_tipe ? (
                                ...
                                <div className="space-y-2 border-t pt-3">
                                    ...
                                    <p className="text-muted-foreground text-xs">
                                        Kalau risikonya baru saja didaftarkan lewat tombol "Input ke Register Risiko" di atas, cari &amp; tautkan di
                                        sini — laporan ini akan bisa dicatat ke Form 10 setelah tertaut.
                                    </p>
                                    <div className="relative">
                                        ...
                                        />
                                        {cariRisikoSearching && <p className="text-muted-foreground mt-1 text-xs">Mencari...</p>}
                            ...
                            {detail.risiko_terdaftar_tipe && (
                                <div className="space-y-2 border-t pt-3">
                                    ...
                                    <p className="text-muted-foreground text-xs">
                                        Buka Form 10 di tab baru dengan risiko, tanggal, sebab, dan dampak sudah terisi otomatis dari laporan ini —
                                        petugas tetap meninjau &amp; melengkapi sebelum menyimpan.
                                    </p>
                            ...
                            <div className="space-y-2 border-t pt-3">
                                ...
                                {detail.ditindaklanjuti_oleh && (
                                    <p className="text-muted-foreground text-xs">Terakhir ditindaklanjuti oleh {detail.ditindaklanjuti_oleh}</p>
                                )}
