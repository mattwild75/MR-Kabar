        ...
        <div className="mt-3 overflow-hidden rounded-md border">
            <table className="w-full text-sm">
                <thead className="bg-muted/50">
                    <tr>
                        <th className="text-muted-foreground px-3 py-2 text-left font-medium">Field</th>
                        {old && <th className="text-muted-foreground px-3 py-2 text-left font-medium">Sebelum</th>}
                        <th className="text-muted-foreground px-3 py-2 text-left font-medium">{old ? 'Sesudah' : 'Nilai'}</th>
                    </tr>
                </thead>
                <tbody>
                    {fields.map((field) => (
                        <tr key={field} className="border-t align-top">
                            ...
                            {old && (
                                <td className="text-muted-foreground decoration-destructive/50 px-3 py-2 whitespace-pre-line line-through">
...
function LogRow({ log }: { log: Activity }) {
    ...
    return (
        <Collapsible open={open} onOpenChange={setOpen}>
            <CollapsibleTrigger className="hover:bg-muted/50 flex w-full items-center gap-3 rounded-md px-3 py-3 text-left">
                ...
                {log.subject_id && <span className="text-muted-foreground shrink-0 text-xs">#{log.subject_id}</span>}

                <span className="text-muted-foreground ml-auto flex shrink-0 items-center gap-2 text-sm">
                    <Avatar className="h-6 w-6">
                        <AvatarFallback className="text-[10px]">{log.causer ? initials(log.causer.name) : 'SYS'}</AvatarFallback>
                    </Avatar>
                    {log.causer?.name ?? 'Sistem'}
                </span>
                <span className="text-muted-foreground shrink-0 text-xs whitespace-nowrap">{formatTanggalWaktu(log.created_at)}</span>
                {fieldCount > 0 && (
...
export default function AuditLogIndex({ logs, filters, filterOptions }: Props) {
    ...
    const applyFilter = (next: Partial<Filters>) => {
        router.get('/audit-logs', { ...filters, ...next }, { preserveState: true, preserveScroll: true, replace: true });
    };
            ...
            <div className="flex-1 space-y-4 p-4 md:p-6">
                ...
                <Card>
                    <CardContent className="flex flex-wrap items-center gap-2 pt-6">
                        ...
                        <Select
                            ...
                            onValueChange={(v) => applyFilter({ subject_type: v === 'all' ? undefined : v })}
                        >
                        ...
                        <Select value={filters.causer_id ?? 'all'} onValueChange={(v) => applyFilter({ causer_id: v === 'all' ? undefined : v })}>
                            <SelectTrigger className="w-[180px]">
                ...
                <Card>
                    <CardContent className="p-2">
                        {logs.data.length === 0 ? (
                            <p className="text-muted-foreground py-12 text-center text-sm">Tidak ada log yang cocok dengan filter ini.</p>
                        ) : (
