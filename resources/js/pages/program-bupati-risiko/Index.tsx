...
const cocok = (teks: string | null | undefined, query: string) => query !== '' && (teks ?? '').toLowerCase().includes(query);
...
export default function ProgramBupatiRisikoIndex({ programs, riskLevels, totalRisikoTerpetakan, visiMisiPemda, bolehHapus, usulan }: PageProps) {
    ...
    const putuskanUsulan = (usulanId: number, keputusan: 'setujui' | 'tolak') => {
        router.post(
            `/program-bupati-risiko/usulan/${usulanId}/${keputusan}`,
            {},
            {
                preserveScroll: true,
    ...
    const filtered = query
        ? programs.filter(
              (p) =>
                  ...
                  p.risiko.some((r) => r.uraian_risiko.toLowerCase().includes(query) || (r.opd_penanggung_jawab ?? '').toLowerCase().includes(query)),
          )
        : programs;
    ...
    return (
        <AppLayout>
            ...
            <div className="space-y-4 p-4">
                ...
                <div className="flex flex-wrap gap-3 text-sm">
                    <div className="bg-card rounded-md border px-3 py-2">
                        ...
                        <span className="text-foreground font-semibold">{programs.length} / 100</span>
                    </div>
                    <div className="bg-card rounded-md border px-3 py-2">
                        <span className="text-muted-foreground">Risiko Terkait: </span>
                        <span className="text-foreground font-semibold">{totalRisikoTerpetakan} risiko</span>
                    </div>
                    <div className="bg-card rounded-md border px-3 py-2">
                        <span className="text-muted-foreground">Total Kaitan Risiko Prioritas: </span>
                        <span className="text-destructive font-semibold">{totalRisikoPrioritas}</span>
                    </div>
                </div>
                ...
                <div className="flex flex-wrap items-center gap-3">
                    ...
                    <Button
                        ...
                        size="sm"
                        className="shrink-0"
                        onClick={() => setExpandedIds(semuaTerbukaSaatIni ? new Set() : new Set(filtered.map((p) => p.id)))}
                    >
                        {semuaTerbukaSaatIni ? (
                            <>
                ...
                {usulan.length > 0 && (
                    <div className="rounded-md border border-sky-500/40 bg-sky-500/5 p-3">
                        ...
                        <div className="space-y-1.5">
                            {usulan.map((u) => (
                                <div key={u.id} className="bg-card flex flex-wrap items-center gap-2 rounded-md px-2 py-1.5 text-sm shadow-sm">
                                    <Badge
                                        variant="outline"
                                        className={`shrink-0 text-[10px] ${u.aksi === 'lepas' ? 'border-red-400 text-red-700 dark:text-red-400' : 'border-emerald-400 text-emerald-700 dark:text-emerald-400'}`}
                                    >
                                        {u.aksi === 'lepas' ? 'Lepas' : 'Tambah'}
                                    </Badge>
                                    <span className="text-muted-foreground shrink-0 text-xs">Program #{u.program_nomor}</span>
                                    <span className="min-w-0 flex-1 truncate">{u.uraian_risiko ?? '(risiko tidak ditemukan)'}</span>
                                    {bolehHapus && (
                                        <>
                                            <span className="text-muted-foreground shrink-0 text-xs">oleh {u.pengusul}</span>
                                            ...
                                            <Button
                                                ...
                                                onClick={() => putuskanUsulan(u.id, 'tolak')}
                                            >
                                                Tolak
                                            </Button>
                                        </>
                            ...
                            <div key={g.urutan} className="space-y-2">
                                ...
                                <div className="space-y-2">
                                    {g.programs.map((program) => {
                                        ...
                                        return (
                                            <div key={program.id} id={`program-card-${program.id}`} className="bg-card rounded-md border">
                                                <button
                                                    type="button"
                                                    onClick={() => toggleExpand(program.id)}
                                                    className="hover:bg-muted/50 flex w-full items-start justify-between gap-3 p-3 text-left"
                                                >
                                ...
                                menyebabkannya muncul. */}
                                                        {query !== '' && (
                                                            <Badge
                                                                variant="outline"
                                                                className="border-yellow-500/60 text-[10px] text-yellow-700 dark:text-yellow-400"
                                                            >
                                                ...
                                                {expanded && (
                                                    <div className="bg-muted/40 ml-4 space-y-1.5 rounded-b-md border-t border-l-2 border-l-sky-500/40 px-3 py-2">
                                                        ...
                                                        {program.risiko.map((r) => (
                                                            <div
                                                                ...
                                                                className={`bg-card hover:bg-muted flex items-center gap-2 rounded-md px-1.5 py-1 text-sm shadow-sm transition-colors ${
                                                                    highlightPivotId === r.pivot_id
                                                                        ? 'bg-amber-200/60 ring-2 ring-amber-400 dark:bg-amber-500/20'
                                                                        : ''
                                                                }`}
                                                            >
                                      ...
                                      ikut ramai. */}
                                                                ...
                                                                {r.program_semua.length > 1 && (
                                                                    <button
                                                                        type="button"
                                                                        onClick={() =>
                                                                            sorotProgramLain(`${r.tipe}#${r.id}`, r.program_semua, program.nomor)
                                                                        }
                                    ...
                                    sedang ditunggu. */}
                                                                    ...
                                                                    <AlertDialog>
                                                                        <AlertDialogTrigger asChild>
                                                                            <Button
                                                                                ...
                                                                                title={
                                                                                    bolehHapus
                                                                                        ? 'Lepas kaitan risiko ini'
                                                                                        : 'Usulkan pelepasan kaitan ini ke Admin'
                                                                                }
                                                                        ...
                                                                        <AlertDialogContent>
                                                                            <AlertDialogHeader>
                                                                                <AlertDialogTitle>
                                                                                    {bolehHapus
                                                                                        ? 'Hapus kaitan risiko ini?'
                                                                                        : 'Usulkan pelepasan kaitan ini?'}
                                                                                </AlertDialogTitle>
                                                                                <AlertDialogDescription>
                                                                                    {bolehHapus ? (
                                                                                        <>
                                                                                            Kaitan ke risiko "{r.uraian_risiko}" akan dihapus dari
                                                                                            Program #{program.nomor} — risiko aslinya di{' '}
                                                                                            {TIPE_LABEL[r.tipe]} TIDAK ikut terhapus, dan kaitan ini
                                                                                            bisa dipulihkan lewat menu Data Terhapus.
                                                                                        </>
                                                                                    ) : (
                                                                                        <>
                                                                                            Usulan pelepasan kaitan ke risiko "{r.uraian_risiko}" dari
                                                                                            Program #{program.nomor} akan dikirim ke Admin. Kaitannya
                                                                                            BELUM dilepas sampai Admin menyetujui, dan Anda akan
                                                                                            menerima notifikasi begitu diputuskan.
                                                                                        </>
                                                                                    )}
                                                                                </AlertDialogDescription>
                                                                            </AlertDialogHeader>
                                                                            <AlertDialogFooter>
                                                                                ...
                                                                                <AlertDialogAction
                                                                                    ...
                                                                                    className={
                                                                                        bolehHapus ? 'bg-destructive hover:bg-destructive/90' : ''
                                                                                    }
                                                                                >
                                                        ...
                                                        <Button
                                                            ...
                                                            size="sm"
                                                            className="mt-1"
                                                            onClick={() => setTambahUntukProgram(program)}
                                                        >
            ...
            <TambahKaitanDialog program={tambahUntukProgram} bolehLangsung={bolehHapus} onClose={() => setTambahUntukProgram(null)} />
        </AppLayout>
    );
}
    ...
    return (
        <Dialog open={program !== null} onOpenChange={(open) => !open && onClose()}>
            <DialogContent className="max-w-lg">
                ...
                <p className="text-muted-foreground text-xs">{program?.program_pembangunan}</p>
                {!bolehLangsung && (
                    <p className="text-muted-foreground rounded-md bg-sky-500/10 px-2.5 py-2 text-xs">
                        Pencarian ini hanya menampilkan risiko dari register OPD Anda sendiri. Pilihan Anda dikirim sebagai usulan dan baru berlaku
                ...
                {searching && <p className="text-muted-foreground text-xs">Mencari...</p>}
                <div className="max-h-72 space-y-1 overflow-y-auto">
                    {hasil.map((r) => (
                        <button
                            ...
                            onClick={() => tambahkan(r)}
                            disabled={saving}
                            className="hover:bg-muted block w-full rounded-md border px-3 py-2 text-left text-sm disabled:opacity-50"
                        >
                    ...
                    {query.trim().length >= 2 && !searching && hasil.length === 0 && (
                        <p className="text-muted-foreground py-2 text-center text-xs">Tidak ada hasil untuk "{query}".</p>
                    )}
                </div>
            </DialogContent>
        </Dialog>
