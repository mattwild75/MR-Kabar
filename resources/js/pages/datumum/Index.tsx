...
import { DAFTAR_SUB_URUSAN, DAFTAR_URUSAN } from '@/lib/urusan-pemerintahan';
            ...
            <form onSubmit={submit} className="space-y-4 p-4">
                ...
                {isAdmin && !belumPilihOpd && targetUserBelumAda && (
                    <p className="rounded-md border border-amber-300 bg-amber-50 p-3 text-sm text-amber-800">
                        OPD ini belum memiliki akun PIC terdaftar — Data Umum tidak dapat diisi/diubah sampai ada akun PIC yang terhubung ke OPD ini.
                    </p>
                )}

                {formAktif && belumAdaData && (
                    <p className="rounded-md border border-amber-300 bg-amber-50 p-3 text-sm text-amber-800">
                        Belum ada Data Umum utk Tahun Penilaian {tahun}. Isi & simpan form di bawah utk melengkapi identitas/PIC/TTD tahun ini — data
                        tahun lain TIDAK ikut ditampilkan di sini.
                    </p>
                )}
                                ...
                                {signatories.map((s, i) => (
                                    <div key={i} className="grid grid-cols-1 gap-2 rounded-md border p-3 sm:grid-cols-[1fr_1fr_1fr_auto]">
                                        <div className="space-y-1">
                                            ...
                                            <Input
                                                value={s.jabatan}
                                                onChange={(e) => updateSignatory(i, 'jabatan', e.target.value)}
                                        ...
                                        <div className="space-y-1">
                                            ...
                                            <Input
                                                value={s.nama}
                                                onChange={(e) => updateSignatory(i, 'nama', e.target.value)}
                                        ...
                                        <div className="space-y-1">
                                            ...
                                            <Input
                                                value={s.nip}
                                                onChange={(e) => updateSignatory(i, 'nip', e.target.value)}
                                                placeholder="NIP"
                                                disabled={isCeeSurvey}
                                            />
                                        </div>
