...
function InherenResidualTooltip({ active, payload }: { active?: boolean; payload?: { payload: InherenResidualItem }[] }) {
    ...
    return (
        <div className="bg-popover text-popover-foreground rounded-md border p-3 text-sm shadow-md">
            ...
            <p className="text-muted-foreground mb-2 text-xs">{item.uraian_risiko}</p>
    ...
    return (
        <Popover open={open} onOpenChange={setOpen}>
            <PopoverTrigger asChild>
                <button type="button" className="-m-1 w-full space-y-1 rounded-md p-1 text-left">
            ...
            <PopoverContent className="w-80">
                <p className="text-muted-foreground mb-2 text-xs font-medium">Rincian tahapan — {item.opd_nama}</p>
                <div className="space-y-1">
                    {item.tahap.map((t) => (
                        ...
                        >
                            ...
                            {!t.selesai && <ChevronRight className="text-muted-foreground h-3.5 w-3.5 shrink-0" />}
                        </button>
                    ))}
                </div>
            </PopoverContent>
...
function MatrixCell({
    ...
    const content = (
        <div
            title={
                `Dampak ${dampak} x Kemungkinan ${kemungkinan} = ${jumlah} risiko (skala ${cell?.skala_risiko ?? '-'})` +
                ...
                <div className="max-h-72 space-y-1 overflow-y-auto">
                    {daftarRisiko.map((r) => (
                        <button
                            ...
                            className="hover:bg-muted flex w-full flex-col gap-0.5 rounded-md px-2 py-1.5 text-left text-sm"
                        >
                            <span className="line-clamp-2 flex-1">{r.uraian_risiko}</span>
                            <span className="text-muted-foreground flex items-center gap-1.5 text-xs">
                                {r.opd_nama} · {r.tipe}
                                {r.rtp_terisi ? <CheckCircle2 className="h-3 w-3 text-green-500" /> : <XCircle className="h-3 w-3 text-amber-500" />}
                            </span>
                        </button>
                    ))}
                </div>
                <p className="text-muted-foreground mt-2 border-t pt-2 text-[11px] leading-snug">
                    Tanda RTP di sini berlaku untuk risiko manapun (semua skala) — beda dari tahap "RTP Risiko Prioritas" di widget Progres Tahapan
                    yang hanya menghitung risiko skala Tinggi/Sangat Tinggi.
                </p>
            </PopoverContent>
        </Popover>
    );
...
export default function Dashboard({
    ...
    const logKejadianBulanOptions = useMemo(
        () => Array.from(new Set(logKejadian.filter((l) => l.tanggal).map((l) => l.tanggal!.slice(0, 7)))).sort((a, b) => b.localeCompare(a)),
        [logKejadian],
    );
        ...
        <AppLayout breadcrumbs={breadcrumbs}>
                ...
                Sama persis dengan halaman Keterangan Pendukung. */}
                        <CardContent className="[--batas-selera:#b91c1c] dark:[--batas-selera:#fca5a5]">
                            <div className="flex gap-2">
                                <div className="text-muted-foreground flex flex-col justify-between py-1 text-xs">
                                    {[5, 4, 3, 2, 1].map((k) => (
                            ...
                            <p className="text-muted-foreground mt-2 text-center text-xs">
                                Dampak (sumbu horizontal) &amp; Kemungkinan (sumbu vertikal) — Skala Risiko dibaca dari tabel peringkat 1–25, bukan
                                hasil perkalian kedua sumbu
                            </p>
                            ...
                            {seleraAmbang !== null && (
                                <div className="text-muted-foreground mt-2 flex flex-wrap items-center gap-x-2 gap-y-1 text-xs">
                                    ...
                                    />
                                    <span className="text-foreground font-medium">Batas Selera Risiko</span>
                                    <span>Skala Risiko {seleraAmbang} ke atas berada di luar selera dan wajib punya Rencana Tindak Pengendalian</span>
                                </div>
                            )}
                        </CardContent>
                    </Card>
                    ...
                    <CardContent>
                        {inherenResidual.length === 0 ? (
                            ...
                            <>
                                        ...
                                        <ResponsiveContainer width="100%" height={Math.max(260, inherenResidual.length * 32)}>
                                            ...
                                            >
                                                ...
                                                <Bar
                                                    ...
                                                    onClick={(data) =>
                                                        setRisikoDetail(
                                                            toMatriksDetailRisiko((data?.payload ?? data) as unknown as InherenResidualItem),
                                                        )
                                                    }
                                                />
                                                <Bar
                                ...
                                <p className="text-muted-foreground mt-2 text-center text-xs">
                                    Menampilkan seluruh {inherenResidual.length} risiko yang sudah memiliki Skala Inheren — bar biru+merah = Residual
                                    & Gap ke Inheren, ◆ hijau = Target (sasaran RTP), ▲ oranye = Aktual (hasil monitoring, bila sudah diisi di Form 9)
                                </p>
                            </>
                        )}
                    </CardContent>
                    ...
                    <Card className="h-full">
                        ...
                        <CardContent>
                            {risikoPrioritas.length === 0 ? (
                                <p className="text-muted-foreground text-sm">Tidak ada risiko prioritas.</p>
                            ) : (
                                <>
                                    <div className="max-h-[420px] space-y-2 overflow-y-auto">
                                        {risikoPrioritas.map((r) => (
                                            <button
                                                ...
                                                className="hover:bg-muted flex w-full items-start justify-between gap-3 rounded-md border px-3 py-2 text-left"
                                            >
                                                ...
                                                <div className="flex shrink-0 flex-col items-end gap-1">
                                                    ...
                                                    <Badge variant={r.rtp_status === 'RTP Tersusun' ? 'default' : 'destructive'}>
                                                        {r.rtp_status}
                                                    </Badge>
                                                </div>
                                            </button>
                        ...
                        {trenEfektivitasPengendalian.every((t) => t.total_dinilai === 0) ? (
                            ...
                            <>
                                    ...
                                    <ResponsiveContainer width="100%" height="100%">
                                        <LineChart data={trenEfektivitasPengendalian}>
                                            ...
                                            <YAxis
                                                ...
                                                allowDecimals={false}
                                                domain={[-10, 25]}
                                                label={{ value: 'Skala', angle: -90, position: 'insideLeft', style: { fontSize: 11 } }}
                                            />
                                            <YAxis
                                                yAxisId="persen"
                                                orientation="right"
                                            ...
                                            <Tooltip
                                                formatter={(value, name) => [
                                                    name === 'Cakupan Signifikan' ? `${Number(value).toFixed(2)}%` : value,
                                                    name,
                                                ]}
                                            />
                                            <Legend />
                                            ...
                                            <Line
                                                ...
                                                label={{
                                                    ...
                                                    fontSize: 11,
                                                    fill: '#0ea5e9',
                                                    formatter: (v: ReactNode) => (typeof v === 'number' ? `${v.toFixed(2)}%` : v),
                                                }}
                                            />
                                            <Line
                                                yAxisId="gap"
                                ...
                                <p className="text-muted-foreground text-center text-xs">
                                    Rata-rata Gap = besaran penurunan skala risiko (Inheren − Residual) · Cakupan Signifikan = % risiko dengan gap ≥{' '}
                                    {5} · Rata-rata Deviasi Target vs Aktual = Aktual − Target pada risiko yang sudah dimonitor di Form 9 (positif =
                                    RTP meleset dari rencana)
                                </p>
                            </>
                        )}
                    </CardContent>
                  ...
                  daftar panjang (~49 OPD) mendorong layout dashboard. */}
                    ...
                    per-risiko), karena skor_total sekarang tidak lagi terbatas 1-25. */}
                                    ...
                                    return rankingOpd.map((r, i) => {
                                        ...
                                        return (
                                            ...
                                            <button
                                                key={r.opd_id}
                                                type="button"
                                                onClick={() => router.get('/data-risiko-gabungan', { opd: r.opd_nama }, { preserveScroll: false })}
                                                title={`Lihat data risiko ${r.opd_nama}`}
                                                className="hover:bg-muted -m-1 w-full space-y-1 rounded-md p-1 text-left transition-colors"
                                            >
                                                <div className="flex items-center justify-between text-sm">
                                                    ...
                                                    <span className="text-muted-foreground text-xs">
                                                        {r.risiko_tinggi} tinggi dari {r.total_risiko} · skor total {r.skor_total} · rata-rata{' '}
                                                        {rata ?? '-'}
                                                    </span>
                                                </div>
                                ...
                                <p className="text-muted-foreground pt-1 text-xs">
                                    Diurutkan berdasar skor total eksposur (Σ skala risiko seluruh risiko OPD) — bukan rata-rata, supaya OPD dengan
                                    banyak risiko sedang tidak kalah prioritas dari OPD ber-1-2 risiko tinggi saja. Klik salah satu baris untuk
                                    membuka daftar risikonya.
                                </p>
                            </CardContent>
                        </Card>
                    )}
                            ...
                            {(() => {
                                ...
                                return (
                                    <>
                                        ...
                                        {filtered.length === 0 ? (
                                            <p className="text-muted-foreground text-sm">Tidak ada kejadian yang cocok dengan filter.</p>
                                        ) : (
                                            <>
                                                <div className="max-h-[420px] space-y-2 overflow-y-auto">
                                                    {filtered.map((l, i) => (
                                                        <div
                                                            key={`${l.sumber}|${l.opd_nama}|${l.tanggal ?? ''}|${l.uraian}|${i}`}
                                                ...
                                                <p className="text-muted-foreground mt-2 text-center text-xs">
                                                    Menampilkan {filtered.length} kejadian
                                                </p>
                                            </>
                                        )}
                                    </>
                                        ...
                                        : k.status === 'sebagian'
                                          ...
                                          : k.status === 'n/a'
                                            ? {
                                                  ...
                                                  label: 'Belum Ada Risiko Teridentifikasi',
                                              }
                                            : {
                                                  icon: XCircle,
                            ...
                            {(() => {
                                ...
                                return (
                                    <>
                                        ...
                                        {filtered.length === 0 ? (
                                            <p className="text-muted-foreground text-sm">Tidak ada aktivitas yang cocok dengan filter.</p>
                                        ) : (
                                            <>
                                                <div className="max-h-[420px] space-y-2 overflow-y-auto">
                                                    {filtered.map((a, i) => (
                                                        <div
                                                            key={`${a.waktu_iso ?? ''}|${a.causer_nama}|${a.deskripsi}|${i}`}
                                                            className="flex items-start gap-3 rounded-md border px-3 py-2"
                                                        >
                                                            ...
                                                            <div className="min-w-0 flex-1">
                                                                ...
                                                                <p className="text-muted-foreground text-xs">{a.waktu}</p>
                                                            </div>
                                                        </div>
                                                    ))}
                                                </div>
                                                <p className="text-muted-foreground mt-2 text-center text-xs">
                                                    Menampilkan {filtered.length} aktivitas
                                                </p>
                                            </>
                                        )}
                                    </>
            ...
            <Dialog open={tahapDetail !== null} onOpenChange={(o) => !o && setTahapDetail(null)}>
                <DialogContent>
                    ...
                    <p className="text-muted-foreground text-sm">
                        Tahap ini belum lengkap untuk <span className="text-foreground font-medium">{tahapDetail?.opdNama}</span>. Buka form terkait
                        untuk melengkapinya.
                    </p>
                    <DialogFooter>
                        {tahapDetail && (
            ...
            <Dialog open={kategoriOpen !== null} onOpenChange={(o) => !o && setKategoriOpen(null)}>
                <DialogContent>
                    ...
                    <p className="text-muted-foreground text-xs">{kategoriOpen?.jumlah} risiko dalam kategori ini</p>
