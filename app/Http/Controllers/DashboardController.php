...
{
    public function __construct(private readonly DasborService $dasbor, private readonly IngatanRingkasanService $ingat) {}
    ...
    public function index(Request $request)
    {
        ...
        // Seluruh widget dihitung sekali per (tahun, OPD, peran) dan diingat
        // sampai salah satu tabel sumbernya berubah (sidik jari tabel di
        // IngatanRingkasanService) — dasbor dibuka jauh lebih sering
        // daripada datanya berubah.
        $tabelSumber = ['tbl_irs_pemda', 'tbl_irs_pd', 'tbl_iro_pd', 'monitoring_rtp', 'pencatatan_kejadian_risiko',
            'cee_simpulan', 'cee_rtp', 'cee_unsur', 'arahan_penilaian_risiko', 'opd', 'risk_levels', 'risk_matrix_cells',
            'pengaturan_pemda', 'activity_log', 'users'];
        $widget = $this->ingat->ingat('dasbor', $tabelSumber, ['tahun' => $tahun, 'opd' => $opdId, 'admin' => $isAdmin], function () use ($tahun, $opdId, $isAdmin) {
