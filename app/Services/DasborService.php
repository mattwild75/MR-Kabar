...
/**
 * Pembangun data Dashboard MR Kabar — HANYA widget yang berdasar langsung pada Perdep PPKD
 ...
 * Admin/Super Admin melihat lintas-OPD — penyaringnya (`scopedOpdId`) tetap di
 * DashboardController, karena hanya itu yang menyentuh Request.
 *
 * DIPISAHKAN DARI CONTROLLER 12 September 2026 (temuan audit R-16). Semua
 * `build*()` adalah fungsi murni dari koleksi baris + tahun/OPD; tidak satu pun
 * menyentuh Request atau auth(). Controller kini hanya merangkai. Dua controller
 * lain masih menyalin sebagian logika ini (CetakLaporanController::
 * buildProgresTahapan, ProgramBupatiRisikoController::collectRiskRows) —
 * irisan berikutnya adalah membuat keduanya memanggil layanan ini.
 */
class DasborService
{
    ...
    public function __construct(public readonly RiskReferenceDataService $riskRef) {}

    /** Aksesor memoized ke collectRiskRows() — semua widget lewat sini. */
    public function rowsForTahun(int $tahun, ?int $opdId): Collection
    {
        $key = $tahun.':'.($opdId ?? 'all');
    ...
    public function pengaturan(): PengaturanPemda
    {
    ...
    public function tahunOptions(?int $opdId): array
    {
     ...
     * kepatuhan).
     */
    public function opdIdsWithRiskRows(int $tahun, ?int $opdId): Collection
    {
        $query = fn ($model) => $opdId
            ? $model::whereHas('user', fn ($q) => $q->where('opd_id', $opdId))
            : $model::query();
    ...
    public function buildRingkasan(Collection $rows, int $ambangTinggi, array $kepatuhan): array
    {
     ...
     */
    public function buildMatriks(Collection $rows): array
    {
        $cells = [];
     ...
     */
    public function buildMatriksDetail(Collection $rows): array
    {
        $cells = [];
        for ($kemungkinan = 1; $kemungkinan <= 5; $kemungkinan++) {
            for ($dampak = 1; $dampak <= 5; $dampak++) {
    ...
    public function buildMatrixCells(): array
    {
     ...
     */
    public function buildProgresTahapan(int $tahun, ?int $opdId, int $ambangTinggi): array
    {
    ...
    public function buildDistribusiTingkat(Collection $rows): array
    {
    ...
    public function buildDistribusiKategori(Collection $rows): array
    {
        return $rows
     ...
     */
    public function buildInherenResidual(Collection $rows): array
    {
        return $rows
     ...
     */
    public function buildRisikoPrioritas(Collection $rows, int $ambangTinggi): array
    {
        return $rows
    ...
    public function buildTrenTahunan(?int $opdId, int $tahunAktif): array
    {
     ...
     */
    public function buildTrenEfektivitasPengendalian(?int $opdId, int $tahunAktif): array
    {
        $tahunList = range($tahunAktif - 4, $tahunAktif);
     ...
     */
    public function buildRankingOpd(int $tahun): array
    {
     ...
     */
    public function buildLogKejadian(?int $opdId): array
    {
     ...
     */
    public function buildJadwalPenilaian(int $tahun): array
    {
     ...
     */
    public function buildKepatuhan(int $tahun, ?int $opdId, Collection $opdIdsWithRiskRows): array
    {
        $opds = $opdId ? Opd::where('id', $opdId)->get() : Opd::orderBy('nama')->get();
    ...
    public function buildActivityFeed(?int $opdId, bool $isAdmin): array
    {
